...
def iteration(values, mean, inside_probability, make_intentional_error, debug=False):
    interval = interval_estimate.estimate_interval(values, inside_probability, make_intentional_error, debug=debug)[0]
...
def evaluate_interval(inside_probability, iterations=100):
    ...
    for i in range(iterations):
        ...
        if iteration(values, mean, inside_probability, make_intentional_error=False):
            positive_count_right += 1
        if iteration(values, mean, inside_probability, make_intentional_error=True):
